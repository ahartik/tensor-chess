    ...
    def __init__(self, is_training, board, move_ind, result):
        ...
        if use_residual:
            ...
            while layers < num_layers - 1:
                conv_output = self.add_res_conv2d(conv_output, 3, 3, depth / 2,
                                                  depth)
                layers += 2
        ...
        self.result_prediction = tf.reshape(
            self.result_prediction, [-1], name="result_prediction")

        self.result_loss = tf.reduce_sum(
            tf.squared_difference(self.result_prediction, result))
        ...
        self.reinforce_loss = -tf.reduce_sum(
            (result - self.result_prediction) * tf.log(tf.clip_by_value(move_prob, 1e-8, 1.0)))
