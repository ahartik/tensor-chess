...
class DecodeBoardOp : public OpKernel {
 public:
  ...
  void Compute(tensorflow::OpKernelContext* ctx) override {
    ...
    Board board_msg;
    const bool can_parse = board_msg.ParseFromArray(as_str.data(), as_str.size());
    ...
    OP_REQUIRES_OK(ctx, output.allocate(2, tensorflow::TensorShape{1}, &result));
